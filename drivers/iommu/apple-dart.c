...
#define DART_SWBYPASS_RAM_START 0x800000000
#define DART_SWBYPASS_IOVA_SIZE (1ULL << 32)
 ...
 * @sw_bypass_domain: domain programmed with a linear mapping for sw bypass
...
struct apple_dart {
	...
	struct iommu_domain *sw_bypass_domain;
 ...
 *
 * The normal variant is used inside apple_dart_master_cfg where we always
 * have some outer synchronization primitive (group lock or domain init mutex).
 ...
 * and never changed again afterwards.
 *
 * @dart dart pointer
 ...
 * @original_dma_range_map: original dma_range_map in case sw bypass is required
 */
struct apple_dart_master_cfg {
	struct apple_dart_stream_map stream_maps[MAX_DARTS_PER_DEVICE];
	const struct bus_dma_region *original_dma_range_map;
};
...
/*
 * Some DARTs (PCIe and Thunderbolt) do not support hardware bypass mode.
 * Additionally, their VA space is limited to 4G and their pagesize is
 * hardcoded to be 16K. When using a 4K CPU page size the kernel wouldn't
 * be able to use hardware attached to these DARTs.
 *
 * To still allow bypass mode we create a static pagetable that maps
 * the entire VA space to the start of RAM at 0x8_0000_0000, which will create a
 * constant offset between CPU and DMA adresses.
 * During device_prove we then force the DMA layer to accommodate the offset
 * by overwriting the device's bus_dma_region.
 */
const struct bus_dma_region bypass_dma_range_map[2] = {
	{
		.cpu_start = DART_SWBYPASS_RAM_START,
		.dma_start = 0,
		.offset = DART_SWBYPASS_RAM_START,
		.size = DART_SWBYPASS_IOVA_SIZE,
	},
	{}
};
...
static void apple_dart_setup_bypass(struct apple_dart_stream_map *stream_map)
{
	struct apple_dart_domain *bypass_domain =
		to_dart_domain(stream_map->dart->sw_bypass_domain);

	if (stream_map->dart->supports_bypass)
		apple_dart_hw_enable_bypass(stream_map);
	else
		apple_dart_setup_translation(bypass_domain, stream_map);
}
				 ...
				 struct device *dev)
{
	...
	case IOMMU_DOMAIN_IDENTITY:
		for_each_stream_map(i, cfg, stream_map)
			apple_dart_setup_bypass(stream_map);
		break;
	}
...
static struct iommu_device *apple_dart_probe_device(struct device *dev)
{
	...
	if (!cfg->stream_maps[0].dart->supports_bypass) {
		cfg->original_dma_range_map = dev->dma_range_map;
		dev->dma_range_map = bypass_dma_range_map;
	}
...
static void apple_dart_release_device(struct device *dev)
{
	...
	if (dev->dma_range_map == bypass_dma_range_map)
		dev->dma_range_map = cfg->original_dma_range_map;
...
static int apple_dart_of_xlate(struct device *dev, struct of_phandle_args *args)
{
	...
	for (i = 0; i < MAX_DARTS_PER_DEVICE; ++i) {
		if (cfg->stream_maps[i].dart == dart) {
			cfg->stream_maps[i].sidmap |= 1 << args->args[i];
			return 0;
		}
	}
	for (i = 0; i < MAX_DARTS_PER_DEVICE; ++i) {
		if (!cfg->stream_maps[i].dart) {
			...
			cfg->stream_maps[i].sidmap = 1 << args->args[0];
			return 0;
		}
	}
...
static int apple_dart_prepare_sw_bypass(struct apple_dart *dart)
{
	...
	dart->sw_bypass_domain =
		apple_dart_domain_alloc(IOMMU_DOMAIN_UNMANAGED);
	if (!dart->sw_bypass_domain)
		return -ENOMEM;

	bypass_domain = to_dart_domain(dart->sw_bypass_domain);
	...
	bypass_domain->pgtbl_ops = alloc_io_pgtable_ops(APPLE_DART, &pgtbl_cfg,
							dart->sw_bypass_domain);
	if (!bypass_domain->pgtbl_ops) {
		ret = -ENOMEM;
		goto error;
	}

	for (offset = 0; offset < DART_SWBYPASS_IOVA_SIZE;
	     offset += dart->pgsize) {
		ret = apple_dart_map(dart->sw_bypass_domain, offset,
				     DART_SWBYPASS_RAM_START + offset,
				     dart->pgsize, IOMMU_WRITE | IOMMU_READ,
				     GFP_KERNEL);
		if (ret)
			goto error;
	}

	bypass_domain->finalized = true;

	return 0;

error:
	apple_dart_domain_free(dart->sw_bypass_domain);
	dart->sw_bypass_domain = NULL;
	return ret;
}
...
static int apple_dart_probe(struct platform_device *pdev)
{
	...
	ret = apple_dart_prepare_sw_bypass(dart);
	if (ret)
		goto err_clk_disable;
